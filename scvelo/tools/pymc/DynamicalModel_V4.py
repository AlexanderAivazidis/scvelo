...
def approx_tau(alpha, beta, gamma, u0, s0, tmax, u, s):
    "Assign time to a point (u,s) given a certain combination of rates and inital conditions"
    
    argument = tt.switch(tt.lt(gamma, beta), (((s - beta/(gamma - beta) * u) - (alpha/gamma - beta/(gamma - beta) * alpha/beta))/
        ((s0 - beta/(gamma - beta) * u0) - (alpha/gamma - beta/(gamma - beta) * alpha/beta))),
              (u - alpha/beta)/(u0 - alpha/beta))
    rate = tt.switch(tt.lt(gamma, beta), gamma, beta)
    argument = tt.switch(tt.lt(0.999, argument), 0.99, argument)
    argument = tt.switch(tt.lt(argument, 0.001), 0.01, argument)
        
    return tt.clip(-1/rate * tt.log(argument), 0, tmax)
...
class DynamicalModel_V4(Pymc3Model):
    r"""
    Dynamic model of splicing kinetics for scRNAseq/snRNAseq data. Version 4
    
    Uses approximate time assignment from the scVelo paper.
    ...
    def __init__(
        ...
        with self.model:
            ...
            self.PK_k = pm.Dirichlet('PK_k', a = self.prior_weights, shape = 4)
            # Rates:
            self.beta_g = 1
            ...
            tmax0 = approx_tau_inf(self.alpha_0g, self.alpha_0g, self.beta_g, self.gamma_g,
                                   ...
                                   eta = 0.999)
            self.t0_g = pm.Uniform('t0_g', lower = 0.01*tmax0, upper = 0.99*tmax0, shape = (1, self.n_genes))
            ...
            tmax2 = approx_tau_inf(self.alpha_2g, self.alpha_0g, self.beta_g, self.gamma_g,
                                   u0, s0, eta = 0.001)
            self.tau0_cg = pm.Deterministic('tau0_cg', approx_tau(self.alpha_0g, self.beta_g, self.gamma_g,
                                                                  self.alpha_2g/self.beta_g, self.alpha_2g/self.gamma_g,
                                                                  self.t0_g, self.u_counts, self.s_counts))
            self.tau2_cg = pm.Deterministic('tau2_cg', approx_tau(self.alpha_2g, self.beta_g, self.gamma_g,
                                                                  tmax2, u0, s0,self.u_counts, self.s_counts))    
        
            # Abundance of unspliced and spliced counts in each state:
            self.mus = mu_mRNA(self.alpha_2g, self.alpha_0g, self.beta_g, self.gamma_g, self.tau0_cg,
    ...
    def compute_latent_state_probs(self):
        ...
        # Abundance of unspliced and spliced counts in each state:
        mus = mu_mRNA(alpha_2g, alpha_0g, beta_g, gamma_g, tau0_cg, tau2_cg, t0_g, l_c, self.n_cells, add_axis = 1)
        a_g = tt.stack([a_u_g, a_s_g], axis = 3)

        probs = classProbs_nbmix_extraDim(mus, PK_k, a_g)(input_data).eval()

        self.samples['post_samples']['state'] = probs
        self.samples['post_sample_means']['state'] = np.mean(probs, axis = 3)
        self.samples['post_sample_sds']['state'] = np.sqrt(np.var(probs, axis = 3))
        self.state = np.argmax(self.samples['post_sample_means']['state'], axis = 2)
        
    def compute_latent_state_probs_deprecated(self, adata):
        """Calculate probability for latent states, given unspliced+spliced counts and dynamic model parameters."""

        input_data = tt.stack([adata.layers['unspliced'].toarray(), adata.layers['spliced'].toarray()], axis = 2)

        alpha_0g = theano.shared(self.samples['post_sample_means']['alpha_0g'])
        alpha_2g = theano.shared(self.samples['post_sample_means']['alpha_2g'])
        beta_g = self.samples['post_sample_means']['beta_g']
        gamma_g = self.samples['post_sample_means']['gamma_g']    
        tau0_cg = self.samples['post_sample_means']['tau0_cg']
        tau2_cg = self.samples['post_sample_means']['tau2_cg']
        PK_k = self.samples['post_sample_means']['PK_k']
        l_c = self.samples['post_sample_means']['l_c']
        a_s_g = self.samples['post_sample_means']['a_s_g']
        a_u_g = self.samples['post_sample_means']['a_u_g']
        t0_g = theano.shared(self.samples['post_sample_means']['t0_g'])
        
        # Abundance of unspliced and spliced counts in each state:
        mus = mu_mRNA(alpha_2g, alpha_0g, beta_g, gamma_g, tau0_cg, tau2_cg, t0_g, l_c, self.n_cells)
        a_g = tt.stack([a_u_g, a_s_g], axis = 2)
        
        adata.obsm['state_probabilities'] = classProbs_nbmix(mus, PK_k, a_g)(input_data).eval()
        
        adata.layers['state'] = np.argmax(adata.obsm['state_probabilities'], axis = 2)
...
def mu_mRNA_oneState(alpha_0g, alpha_1g, beta_g, gamma_g, tau0_cg, tau2_cg, l_c, n_cells, add_axis = 0):
    ''' Calculates expected value of mRNA count in each of four possible states (Induction transient sate, induction steady state,
    repression transient state, repression steady state. Needs rates, latent times, switch time and sequencing depth parameter.'''
    
    # State specific latent times:
    t0_g = (tau0_cg - tau2_cg)[0,:].reshape(gamma_g.shape)
    ...
    return tt.stack([tt.switch(tt.lt(0, tau2_cg), mu2[:,:,0], mu0[:,:,0]),
           tt.switch(tt.lt(0, tau2_cg), mu2[:,:,1], mu0[:,:,1])], axis = 2) 
        
class DynamicalModel_V10(Pymc3Model):
    r"""
    Dynamic model of splicing kinetics for scRNAseq/snRNAseq data. Version 8
    
    Infers time using factorization. Uses global time parameters.
    ...
    def __init__(
        ...
        with self.model:
            ...
            # Time:
            self.T_c_max = pm.Gamma('T_c_max', mu = 100, sigma = 50, shape = 1)
            self.T_c = pm.Uniform('T_c', lower = 0, upper = self.T_c_max, shape = (self.n_cells, 1))
            self.h_gf = pm.Dirichlet('h_gf', a = np.ones(self.n_factors) * 0.1, shape=(self.n_genes, self.n_factors))
            
            self.t0_f = pm.Uniform('t0_f', lower = 0, upper = self.T_c_max, shape=(1, self.n_factors))
            self.t2_f = pm.Uniform('t2_f', lower = self.t0_f, upper = self.T_c_max, shape=(1, self.n_factors))
            
            self.tau0_cg =  pm.Deterministic('tau0_cg', pm.math.dot(self.T_c - self.t0_f, self.h_gf.T))
            self.tau2_cg =  pm.Deterministic('tau2_cg', pm.math.dot(self.T_c - self.t2_f, self.h_gf.T))
            
            # Abundance of unspliced and spliced counts:
            self.mu = pm.Deterministic('mu', mu_mRNA_oneState(self.alpha_2g, self.alpha_0g, self.beta_g, self.gamma_g, self.tau0_cg,
                                                              self.tau2_cg, self.l_c, self.n_cells))

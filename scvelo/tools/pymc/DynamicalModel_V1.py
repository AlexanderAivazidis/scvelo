...
class DynamicalModel_V1(Pymc3Model):
    r"""
    Dynamic model of splicing kinetics for scRNAseq/snRNAseq data. Version 1
    
    Infers time, using a uniform prior that depends on rates. Infers one overdispersion parameter for each gene
    but it is the same for spliced and unspliced counts.
    ...
    def __init__(
        ...
        with self.model:
            ...
            self.a_u_g = pm.Deterministic('a_u_g', 1 / (gene_E_g.T * gene_E_g.T)) 
            self.a_s_g = pm.Deterministic('a_s_g', 1 / (gene_E_g.T * gene_E_g.T))

...
def mu_mRNA_twoState(alpha_0g, alpha_1g, beta_g, gamma_g, tau0_cg, tau2_cg, t0_g, l_c, n_cells, add_axis = 0):
    ''' Calculates expected value of mRNA count in each of two possible states (Induction transient sate,
    repression transient state. Needs rates, latent times, switch time and sequencing depth parameter.'''
...
class DynamicalModel_V5(Pymc3Model):
    r"""
    Dynamic model of splicing kinetics for scRNAseq/snRNAseq data. Version 3
    
    Uses only two states and infers time with exponential priors.
    ...
    def __init__(
        ...
        with self.model:
            ...
            # Time:
            self.t0_g = pm.Exponential('t0_g', lam = 1, shape = (1, self.n_genes))
            self.tau0_cg = pm.Exponential('tau0_cg', lam = 1, shape = (self.n_cells, self.n_genes))
            u0, s0 = repression_initialState(self.alpha_2g, self.alpha_0g, self.beta_g, self.gamma_g, self.t0_g, self.n_cells)
            self.tau2_cg = pm.Exponential('tau2_cg', lam = 1, shape = (self.n_cells, self.n_genes))

...
class DynamicalModel_V9(Pymc3Model):
    ...
    def __init__(
        ...
        with self.model:
            ...
            # Time:
            self.h_gf = pm.Dirichlet('h_gf', a = np.ones(self.n_factors) * 10**(-6), shape=(self.n_genes, self.n_factors))
            self.tmax_f = pm.Gamma('w_hyp', mu = 50, sigma = 25, shape=(1, self.n_factors))
            self.w_cf = pm.Uniform('w_cf', lower = 0, upper = 1 + self.tmax_f, shape=(self.n_cells, self.n_factors))
            self.t0_f = pm.Uniform('t0_f', lower = 0, upper = 1 + self.tmax_f, shape=(1, self.n_factors))
            
            self.tau0_cg =  pm.Deterministic('tau0_cg', pm.math.dot(self.w_cf, self.h_gf.T))
            self.tau2_cg =  pm.Deterministic('tau2_cg', pm.math.dot(self.w_cf - self.t0_f, self.h_gf.T))

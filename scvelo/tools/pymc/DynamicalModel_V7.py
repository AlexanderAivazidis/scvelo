...
def mu_mRNA_oneState(alpha_0g, alpha_1g, beta_g, gamma_g, tau_cg, t0_g, l_c, n_cells, add_axis = 0):
    ''' Calculates expected value of mRNA count in each of four possible states (Induction transient sate, induction steady state,
    repression transient state, repression steady state. Needs rates, latent times, switch time and sequencing depth parameter.'''
    
    # State specific latent times:
    tau0_cg = tt.clip(tau_cg - 0, 0, 10**(6))
    tau2_cg = tt.clip(tau_cg - t0_g, 0, 10**(6))
...
class DynamicalModel_V7(Pymc3Model):
    r"""
    Dynamic model of splicing kinetics for scRNAseq/snRNAseq data. Version 7
    ...
    def __init__(
        ...
        with self.model:
            ...
            self.beta_g = pm.Uniform('beta_g', lower = 1, upper = 10, shape = (1, self.n_genes)) 
...
#             self.tau_cg = pm.Uniform('tau_cg', lower = 0, upper = self.t0_g + self.tmax2, shape = (self.n_cells, self.n_genes))         
            ...
            self.tau_cg = pm.Uniform('tau_cg', lower = 0, upper = self.tau_max, shape = (self.n_cells, self.n_genes))
            ...
            phi_hyp_g = pm.Gamma('phi_hyp_g', mu=9.5,
                                    sigma=1, shape=1)
            gene_E_g = pm.Exponential('gene_E_g', phi_hyp_g, shape = 1)           
            self.a_u_g = pm.Deterministic('a_u_g', 100 + tt.extra_ops.repeat(1 / (gene_E_g.T * gene_E_g.T), self.n_genes, axis = 0).reshape((1, self.n_genes)))
            self.a_s_g = pm.Deterministic('a_s_g', 100 + tt.extra_ops.repeat(1 / (gene_E_g.T * gene_E_g.T), self.n_genes, axis = 0).reshape((1, self.n_genes)))                       

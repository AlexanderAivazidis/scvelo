...
class DynamicalModel_V6(Pymc3Model):
    r"""
    Dynamic model of splicing kinetics for scRNAseq/snRNAseq data. Version 0
    
    Infers time, using a uniform prior that depends on rates. 
    Infers only one overdispersion parameters for all genes that is the same for both spliced and unspliced counts,
    plus: it is bounded at a = 10
    ...
    def __init__(
        ...
        with self.model:
            ...
            phi_hyp_g = pm.Gamma('phi_hyp_g', mu=10,
                                    sigma=1, shape=1)
            gene_E_g = pm.Bound(pm.Exponential, upper = np.sqrt(0.1))('gene_E_g', phi_hyp_g, shape = 1)
            
            self.a_u_g = pm.Deterministic('a_u_g', tt.extra_ops.repeat(1 / (gene_E_g.T * gene_E_g.T), self.n_genes, axis = 0).reshape((1, self.n_genes)))
            self.a_s_g = pm.Deterministic('a_s_g', tt.extra_ops.repeat(1 / (gene_E_g.T * gene_E_g.T), self.n_genes, axis = 0).reshape((1, self.n_genes)))
    ...
    def compute_mean_expression(self):
        ...
        # Abundance of unspliced and spliced counts in each state:
        self.mus = mu_mRNA(alpha_2g, alpha_0g, beta_g, gamma_g, tau0_cg, tau2_cg, t0_g, l_c, self.n_cells, add_axis = 1)
